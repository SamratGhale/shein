...
const Clothes = {
	...
	async getById(_id) {
		...
		const ret = await ClothesModel.aggregate([
		...
		if (ret.length === 0) {
			throw { message: `Item with id ${_id} dosen't exist`, code: 404 };
		}
		if (fs.existsSync(`./modules/clothes/images/${ret[0]._id}`)) {
			ret[0].files = fs.readdirSync(`./modules/clothes/images/${ret[0]._id}`);
		} else {
			ret[0].files = []
		}
		return ret[0];
