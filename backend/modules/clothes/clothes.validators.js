...
module.exports = {
  ...
  register: {
    payload: Joi.object({
      ...
      description: Joi.array().items(Joi.string()).description("Item's description"),
      tags: Joi.array().items(Joi.string()).description("Item's tags"),
      vat: Joi.number().optional().description("Vat amount"),
      images: Joi.array().items(Joi.any().meta({ swaggerType: 'file' })).description('Cloth images'),
    }),
  },
  update: {
    payload: Joi.object({
